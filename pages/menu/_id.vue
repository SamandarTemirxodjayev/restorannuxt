<template>
  ...
  <div class="bg-white rounded-lg overflow-hidden" v-else>
    <div class="fixed bg-white h-12 w-12 text-center items-center justify-center flex rounded-full left-[6px] top-3">
      <nuxt-link to="/home">
        <img :src="require(`@/assets/back.svg`)" alt="My SVG File" height="20px" width="20px">
      </nuxt-link>
    </div>
          ...
          <nuxt-link :to="`/menu/${menuItem._id}`">
            <div v-if="id != menuItem._id">
              <div class="flex items-center p-4">
                <img :src="`${menuItem.photo_url}`" alt="" class=" h-24 w-32 object-cover rounded-lg">
                <div class="ml-4">
                  <h2 class="text-xl font-bold">{{menuItem.name}}</h2>
                  <p class="text-gray-500 text-sm">{{Number(menuItem.price).toLocaleString()}} so'm</p>
                </div>
              </div>
            </div>
          </nuxt-link>
